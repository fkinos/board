...
public class Server {
    ...
    private void HandleClient(
        ...
        while (true) {
            ...
            if (Regex.IsMatch(clientMessage, "^GET", RegexOptions.IgnoreCase)) {
                byte[] response = GenerateHandShakeResponse(clientMessage);
                stream.Write(response, 0, response.Length);
                continue;
            }

            (string decodedMessage, EOpcodeType messageType) = HandleMessage(clientBytes);
            handler(conn, decodedMessage, messageType);
        }
    }

    private (string decodedMessage, EOpcodeType messageType) HandleMessage(byte[] clientBytes) {
        ...
        (ulong offset, ulong messageLength) = DetermineMessageLength(payloadLength, clientBytes.Skip(2));
    ...
    private (ulong offset, ulong messageLength) DetermineMessageLength(
        ulong payloadLength,
    ...
    private byte[] GenerateHandShakeResponse(string message) {
